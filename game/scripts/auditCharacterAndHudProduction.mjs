...
for(const id of [...humans,...zombies]){const e=manifest.characters.find(c=>c.characterId===id);if(!e)fail.push(`missing character ${id}`);else{if(e.frameWidth!==80||e.frameHeight!==112||e.footLine!==104)fail.push(`invalid geometry ${id}`);const required=humans.includes(id)?['idle','walk','aim','shoot','reload','melee','hurt','death','climb','interact']:['idle','walk','attack','hurt','death'];for(const a of required)if(!e.animations[a]||e.animations[a].endFrame-e.animations[a].startFrame<1)fail.push(`invalid animation ${id}:${a}`);}}
