...
test('animation geometry has one normalized 80x112 foot line',()=>{for(const c of manifest.characters){assert.equal(c.frameWidth,80);assert.equal(c.frameHeight,112);assert.equal(c.footLine,104);for(const a of Object.values(c.animations))assert.ok(a.endFrame>a.startFrame)}});
