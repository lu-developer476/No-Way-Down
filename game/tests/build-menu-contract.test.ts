...
test('build identity includes deploy provenance and package version', () => { const info=createBuildInfo({},'test',new Date('2026-01-02T03:04:05Z'),unknown); for (const key of ['frontendSha','branch','buildId','builtAt','canonicalNodeCount','generatedArtCount','packageVersion']) assert.ok(key in info); assert.equal(info.packageVersion,packageJson.version); assert.equal(info.canonicalNodeCount,35); assert.equal(new Date(info.builtAt).toISOString(),info.builtAt) });
test('build SHA priority and fallbacks are deterministic', () => { assert.equal(resolveBuildSha({NWD_BUILD_SHA:'nwd',GITHUB_SHA:'github',RENDER_GIT_COMMIT:'render'},unknown),'nwd'); assert.equal(resolveBuildSha({GITHUB_SHA:'github',RENDER_GIT_COMMIT:'render'},unknown),'render'); assert.equal(resolveBuildSha({GITHUB_SHA:'github'},unknown),'github'); assert.equal(resolveBuildSha({},unknown),'unknown'); assert.equal(createBuildInfo({NWD_BUILD_SHA:'123456789'},'production',new Date(),unknown).shortSha,'1234567') });
