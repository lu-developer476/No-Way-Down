...
const allowedProperties=new Set(['assetKey','depth','scrollFactorX','scrollFactorY','tint','alpha','blendMode','visualRole','occludesActors','lightAnchorId','reflectionStrength','editorOnly','runtimeOnly','qualityTier','sector']);
...
export class TiledVisualRenderer{
 build(scene:Phaser.Scene,map:TiledMap):TiledVisualResult{const objects:Phaser.GameObjects.Image[]=[],layers:string[]=[],missing=new Set<string>();let background=0,architecture=0,props=0,foreground=0;
  for(const layer of map.layers){const defaultRole=VISUAL_LAYER_ROLES[layer.name];if(!defaultRole||!layer.visible||propertyLayer(layer,'editorOnly')===true)continue;let rendered=false;
   for(const object of layer.objects){if(!object.visible||property(object,'editorOnly')===true||property(object,'runtimeOnly')===false)continue;for(const item of object.properties??[])if(!allowedProperties.has(item.name))throw new Error(`UNKNOWN_VISUAL_PROPERTY: ${layer.name}.${object.name}.${item.name}`);const key=property(object,'assetKey');if(typeof key!=='string')throw new Error(`VISUAL_ASSET_KEY_REQUIRED: ${layer.name}.${object.name}`);const asset=existingAsset(key);if(!asset){missing.add(key);continue}const role=property(object,'visualRole')??defaultRole;if(typeof role!=='string'||!asset.allowedRoles.includes(role as VisualAssetRole))throw new Error(`VISUAL_ROLE_NOT_ALLOWED: ${key}.${String(role)}`);if(!scene.textures.exists(key)){missing.add(key);continue}
