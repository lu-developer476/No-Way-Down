...
export class MainMenuScene extends Phaser.Scene {
  ...
  private buildBackground(): void {
    ...
    if (this.textures.exists('nwd-menu')) {
      this.add.image(width / 2, height / 2, 'nwd-menu')
  ...
  private buildControlsPanel(): void {
    ...
    const panelArtwork = this.textures.exists('nwd-menu')
      ? this.add.image(width / 2, height / 2, 'nwd-menu').setDisplaySize(panelWidth, panelHeight).setAlpha(0.62)
  ...
  private buildSetupPanel(): void {
    ...
    const setupArtworkKey = this.textures.exists('nwd-characters')
      ? 'nwd-characters'
      : 'nwd-menu';

    const overlay = this.add.rectangle(width / 2, height / 2, width, height, visualTheme.palette.uiPanelShadow, 0.78);
    const panel = this.add.rectangle(width / 2, height / 2, panelWidth, panelHeight, visualTheme.palette.uiPanelFrame, 0.96)
      .setStrokeStyle(3, visualTheme.palette.uiPanelFrameSoft, 1);
    const panelArtwork = this.textures.exists(setupArtworkKey)
      ? this.add.image(width / 2, height / 2, setupArtworkKey).setDisplaySize(panelWidth, panelHeight).setAlpha(0.7)
      : this.add.rectangle(width / 2, height / 2, panelWidth, panelHeight, 0x1b1522, 0.9);
    const panelTint = this.add.rectangle(width / 2, height / 2, panelWidth, panelHeight, visualTheme.palette.uiPanelTint, 0.68);
    const panelBorder = this.add.rectangle(width / 2, height / 2, panelWidth, panelHeight, 0x000000, 0)
      .setStrokeStyle(3, visualTheme.palette.uiPanelFrameSoft, 1);
    ...
    this.setupPanel = this.add.container(0, 0, [overlay, panel, panelArtwork, panelTint, panelBorder, title, subtitle, ...this.setupOptionTexts, this.setupHintText])
      .setVisible(false)
      .setDepth(20);
  ...
  private handleResize(): void {
    ...
    if (this.setupPanel) {
      const [overlay, panel, panelArtwork, panelTint, panelBorder, title, subtitle, ...rest] = this.setupPanel.list as Phaser.GameObjects.GameObject[];
