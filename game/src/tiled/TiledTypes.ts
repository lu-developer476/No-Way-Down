...
export interface TiledObject { id: number; name: string; type: string; x: number; y: number; width: number; height: number; rotation: number; visible: boolean; gid?:number; properties?: TiledProperty[]; polygon?: TiledPoint[]; polyline?: TiledPoint[]; image?: string }
export interface TiledObjectLayer { id: number; name: string; type: 'objectgroup'; visible: boolean; opacity: number; objects: TiledObject[]; properties?: TiledProperty[]; locked?: boolean; offsetx?:number; offsety?:number; parallaxx?:number; parallaxy?:number }
